...
- **チャンネル**: `@aichan` とメンションすると応答します（`MATTERMOST_AUTO_REPLY_CHANNELS`
  指定チャンネルはメンション不要）。
- **停止**: 実行中に `stop` を送ると現在のタスクを中断します。
- **セッションリセット**: `リセット` `reset` `clear` `newchat` で会話を最初から始めます。

> ⚠️ **Mattermost の仕様**: `/` で始まるメッセージは Mattermost が自前のスラッシュコマンドとして
> 横取りするため、`/stop` や `/reset` は Bot に届きません（「Command not found」になる）。
> 停止・リセットは上記の**スラッシュ無し**の語を使ってください。
> （Mattermost ネイティブの `/` コマンド対応は follow-up 予定。）
- **ファイル**: 応答に `MEDIA:/絶対パス` が含まれると、そのファイルを添付として投稿します。
  ユーザーが添付したファイルもダウンロードしてエージェントに渡されます。

## 並列実行と会話の境界

- **DM**: チャンネル単位で 1 会話（メモリを継続）。
- **チャンネル**: **スレッド単位**。別スレッドは並列に実行され、会話メモリも独立します。
  トップレベル発言はそれぞれ自分のスレッドになるため、共有チャンネルで複数人が
  別々に話しかけても互いに待たされません。
- 同じスレッド内の連投だけは順番に処理されます（文脈の順序を保つため）。
